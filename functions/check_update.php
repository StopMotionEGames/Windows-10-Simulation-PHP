...
function checkForUpdates($currentVersion, $debug, &$logs)
{
  ...
  // Filtrar pelo release mais recente que corresponda ao branch na descrição
  $latestRelease = null;
  foreach ($releasesData as $release) {
    $descriptionLines = explode(PHP_EOL, $release['body']);
    $releaseBranch = isset($descriptionLines[0]) ? trim($descriptionLines[0]) : '';
    if ($releaseBranch === $currentVersion['branch']) {
  ...
  if (!$latestRelease) {
    $logs .= "Nenhum release encontrado que corresponda ao branch: " . $currentVersion['branch'] . "\n";
    return ['logs' => $logs];
  }

  // Extrair as linhas da descrição
  $branch = isset($descriptionLines[0]) ? trim($descriptionLines[0]) : '';
  $version = isset($descriptionLines[1]) ? trim($descriptionLines[1]) : '';
  $compilation = isset($descriptionLines[2]) ? trim($descriptionLines[2]) : '';
  $update = isset($descriptionLines[3]) ? trim($descriptionLines[3]) : '';
